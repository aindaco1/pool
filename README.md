...
Current release branch milestone: **v1.2.15**. Pool pins immutable `dust-wave-platform` `v0.28.0`; its release-evidence scripts now use Release Core for cache-policy auditing, Cloudflare admin response-rule management, and assisted screen-reader evidence while Pool retains production targets, route and accessibility policy, credentials, deployment authority, and one-commit rollback.
