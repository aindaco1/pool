...
Current release branch milestone: **v1.2.8**. Pool pins immutable `dust-wave-platform` `v0.21.0` and consumes its pure inventory reservation/snapshot mechanics with an explicit stored-campaign-authoritative policy. Pool retains Durable Object transactions, KV projection, catalog and checkout rules, pledge effects, deployment, and one-commit rollback authority.
