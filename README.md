...
Current release branch milestone: **v1.2.6**. Pool pins immutable `dust-wave-platform` `v0.18.0` and consumes deterministic Shipping Core profiles, aggregation, fallback/manual quote shapes, and option selection while retaining campaign rules, destination validation, USPS credentials/transport/cache/backoff, checkout, fulfillment, storage, and deployment authority.
