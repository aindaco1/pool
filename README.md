...
Current release branch milestone: **v1.2.3**. Pool now also consumes the characterized cryptographic, token, cookie, email-normalization, and constant-work comparison primitives from its immutable `dust-wave-platform` `v0.15.0` pin. Pool retains its authentication policy, session records, CSRF rules, roles, routes, secrets, storage, and deployment authority.
