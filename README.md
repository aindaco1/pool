...
Current release branch milestone: **v1.2.12**. Pool pins immutable `dust-wave-platform` `v0.25.0` and consumes its bounded durable-outbox mechanics through a thin campaign email adapter. Pool retains KV ownership, templates, suppression, provider sends, pledge effects, credentials, deployment, and one-commit rollback authority.
