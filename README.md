...
[`_config.local.yml`](_config.local.yml) is now intentionally thin: it should only carry true local overrides like localhost URLs, `show_test_campaigns`, and local-only public Turnstile key blanks, not a second copy of the base config.
...
- Marketing saved referral codes write one campaign-scoped KV record only when explicitly saved; QR previews, media-library picker reads, reporting loads, abandoned-checkout health reads, and local Blast drafts do not write KV. Shared Marketing/Blast draft saves write one 7-day campaign-scoped draft record only on explicit save, and stale saves fail on revision conflict. Live Blast sends write the required audit event, and sent history reads recent audit records only when the Blast subtab is opened.
...
- normal dashboard reads, protected preview rendering, content previews, report previews/downloads, supporter filters, analytics views, marketing URL building, media-library picker loads, marketing reporting, abandoned-checkout health, and local editor drafts are designed to add zero KV writes
