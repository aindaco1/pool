...
Current release branch milestone: **v1.2.11**. Pool pins immutable `dust-wave-platform` `v0.24.0` and consumes its bounded Zip-Tax/New Mexico lookup transport through a thin campaign tax adapter. Pool retains provider and fallback selection, taxability, quote calculation, credentials, checkout effects, deployment, and one-commit rollback authority.
