...
import { executeBackupRetention, planBackupRetention } from '../../scripts/pool-backup-retention.mjs';
...
function writeSnapshot(root: string, name: string, createdAt: string, options: { release?: boolean; valid?: boolean } = {}) {
  ...
  fs.writeFileSync(path.join(directory, 'pool-backup.tar.gz.age'), archive, { mode: 0o600 });
  fs.writeFileSync(path.join(directory, 'manifest.json'), `${JSON.stringify({
    version: 2,
    createdAt,
    completedAt: createdAt,
    encrypted: true,
    archive: 'pool-backup.tar.gz.age',
...
describe('backup retention safeguards', () => {
  it('plans by retention buckets while preserving newest, release, and invalid snapshots', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-backup-retention-'));
    ...
    const execution = executeBackupRetention(plan, { root, acknowledge: 'POOL_BACKUP_RETENTION_PRUNE' });
  ...
  it('requires an explicit existing retention root', () => {
    ...
    expect(() => planBackupRetention({ root: path.join(os.tmpdir(), 'missing-pool-backups') })).toThrow(/existing directory/i);
  });

  it('revalidates protection rules immediately before deletion', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-backup-retention-revalidate-'));
    writeSnapshot(root, 'latest', '2026-07-09T04:00:00.000Z');
  ...
  it('rejects a symlinked retention root', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-backup-retention-link-'));
