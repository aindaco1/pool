...
async function makeTempDir() {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'pool-minify-assets-'));
...
describe('site asset minification', () => {
  ...
  it('targets generated CSS and JS assets only', () => {
    ...
    expect(isMinifiableAssetPath('_site/assets/js/campaign.js')).toBe(true);
    expect(isMinifiableAssetPath('_site/assets/js/campaign.js.map')).toBe(false);
  ...
  it('minifies local identifiers without rewriting global names', async () => {
    const source = `
      window.PoolExample = window.PoolExample || {};
      function verboseGlobalName(value) {
        return value ? 1 : 0;
      }
      window.PoolExample.verboseGlobalName = verboseGlobalName;
    `;
    ...
    expect(minified).toContain('window.PoolExample');
  ...
  it('rewrites generated assets only when the minified output is smaller', async () => {
    ...
    await fs.writeFile(jsPath, 'window.PoolApp = window.PoolApp || {};\nwindow.PoolApp.ready = true;\n');
    await fs.writeFile(cssPath, '.example {\n  color: #ffffff;\n  margin: 0px;\n}\n');
    await fs.writeFile(vendorPath, 'function vendorName() {\n  return true;\n}\n');
    await fs.writeFile(sharedPath, 'window.PoolShell = window.PoolShell || {};\nwindow.PoolShell.ready = true;\n');
