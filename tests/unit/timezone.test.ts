import { describe, expect, it } from 'vitest';

import {
  campaignDeadlineDate,
  campaignStartDate,
  getPlatformDateKey,
  getPlatformTimeParts,
  getPlatformTimeZone,
  getSupportedTimeZones,
  getTimeZoneOptions,
  isCampaignDeadlinePassed,
  isSupportedTimeZone
} from '../../worker/src/timezone.js';

describe('platform timezone utilities', () => {
  it('defaults to America/Denver and exposes IANA timezone options', () => {
    expect(getPlatformTimeZone({})).toBe('America/Denver');
    expect(isSupportedTimeZone('America/Denver')).toBe(true);
    expect(isSupportedTimeZone('Not/AZone')).toBe(false);
    expect(getSupportedTimeZones()).toContain('America/Denver');
    expect(getSupportedTimeZones()).toContain('UTC');
    const options = getTimeZoneOptions();
    expect(options).toEqual(expect.arrayContaining([
      { value: 'America/Denver', label: 'America/Denver' },
      { value: 'UTC', label: 'UTC' },
      { value: 'Europe/London', label: 'Europe/London' }
    ]));
    expect(options).toContainEqual({
      value: 'Africa/Addis_Ababa',
      label: 'Africa/Addis Ababa'
    });
  });

  it('converts campaign date boundaries in the configured timezone', () => {
    expect(campaignStartDate('2026-04-21', { PLATFORM_TIMEZONE: 'America/Denver' }).toISOString())
      .toBe('2026-04-21T06:00:00.000Z');
    expect(campaignDeadlineDate('2026-04-21', { PLATFORM_TIMEZONE: 'America/Denver' }).toISOString())
      .toBe('2026-04-22T05:59:59.000Z');

    expect(campaignStartDate('2026-04-21', { PLATFORM_TIMEZONE: 'Asia/Tokyo' }).toISOString())
      .toBe('2026-04-20T15:00:00.000Z');
    expect(campaignDeadlineDate('2026-04-21', { PLATFORM_TIMEZONE: 'Asia/Tokyo' }).toISOString())
      .toBe('2026-04-21T14:59:59.000Z');
  });

  it('derives local date keys and deadline status from the platform timezone', () => {
    const instant = new Date('2026-04-21T05:30:00.000Z');
    expect(getPlatformDateKey({ PLATFORM_TIMEZONE: 'America/Denver' }, instant)).toBe('2026-04-20');
    expect(getPlatformDateKey({ PLATFORM_TIMEZONE: 'Europe/London' }, instant)).toBe('2026-04-21');
    expect(getPlatformTimeParts({ PLATFORM_TIMEZONE: 'America/New_York' }, instant)).toMatchObject({
      year: 2026,
      month: 4,
      day: 21,
      hour: 1,
      minute: 30
    });

    expect(isCampaignDeadlinePassed('2026-04-21', { PLATFORM_TIMEZONE: 'Asia/Tokyo' }, new Date('2026-04-21T15:00:00.000Z'))).toBe(true);
    expect(isCampaignDeadlinePassed('2026-04-21', { PLATFORM_TIMEZONE: 'America/Denver' }, new Date('2026-04-21T15:00:00.000Z'))).toBe(false);
  });

  it('preserves campaign aliases across 23-hour and 25-hour Denver days', () => {
    expect(campaignStartDate('2026-03-08', 'America/Denver').toISOString())
      .toBe('2026-03-08T07:00:00.000Z');
    expect(campaignDeadlineDate('2026-03-08', 'America/Denver').toISOString())
      .toBe('2026-03-09T05:59:59.000Z');
    expect(campaignStartDate('2026-11-01', 'America/Denver').toISOString())
      .toBe('2026-11-01T06:00:00.000Z');
    expect(campaignDeadlineDate('2026-11-01', 'America/Denver').toISOString())
      .toBe('2026-11-02T06:59:59.000Z');
  });
});
