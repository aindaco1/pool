...
describe('worker validation security helpers', () => {
  it('does not default private CORS responses to wildcard origins', () => {
    expect(getAllowedOrigin({}, false)).toBe('https://pool.dustwave.xyz');
    expect(getAllowedOrigin({ CORS_ALLOWED_ORIGIN: '*' }, false)).toBe('https://pool.dustwave.xyz');
    expect(getAllowedOrigin({ CORS_ALLOWED_ORIGIN: 'not a URL' }, false)).toBe('https://pool.dustwave.xyz');
  ...
  it('preserves the Pool JSON response and baseline security-header contract', async () => {
    const response = jsonResponse({ ok: true }, 201, { SITE_BASE: 'https://pool.example/path' });

    expect(response.status).toBe(201);
    expect(response.headers.get('content-type')).toBe('application/json');
    expect(response.headers.get('access-control-allow-origin')).toBe('https://pool.example');
