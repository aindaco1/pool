...
import { collectBackupReadiness } from '../../scripts/pool-backup-readiness.mjs';
...
describe('backup and recovery readiness evidence', () => {
  it('reports only names/status and accepts current sanitized evidence', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-backup-readiness-'));
  ...
  it('fails required missing credentials and stale required evidence', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-backup-readiness-stale-'));
