...
const globalName = 'PoolVideoPosters';
...
describe('first-frame poster characterization', () => {
  it('waits for DOM readiness and intersection, then captures once at bounded dimensions', () => {
    ...
    expect(new URL(f.previews[0].src).searchParams.has('pool_first_frame_poster')).toBe(true);
  ...
  it('retains this consumer\'s document-base and opaque-origin policy', () => {
    const base = fixture({ baseURI: 'https://other.test/base/' }); base.video.currentSrc = 'relative.webm'; base.trigger(); expect(base.previews).toHaveLength(0);
    const opaque = fixture({ origin: 'null' }); opaque.trigger(); expect(opaque.previews).toHaveLength(1);
  });
});
