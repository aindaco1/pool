...
describe('Stripe Worker client integrity', () => {
  ...
  it('pins the API version, carries idempotency, flattens metadata, and emits redacted observations', async () => {
    ...
    await stripe.paymentIntents.create({ amount: 1200, currency: 'usd', metadata: { orderId: 'order-1' } }, { idempotencyKey: 'settle:order-1' });
    ...
    expect(headers.get('Idempotency-Key')).toBe('settle:order-1');
  ...
  it('normalizes Stripe decline details for durable recovery decisions', async () => {
    ...
    await expect(stripe.paymentIntents.create({ amount: 1200, currency: 'usd' }, { idempotencyKey: 'settle:failed' }))
  ...
  it('classifies no-response failures as retryable without claiming an object id', async () => {
    ...
    const stripe = createStripeClient('sk_test_secret');
    await expect(stripe.customers.create({ email: 'supporter@example.com' }, { idempotencyKey: 'customer:order-1' }))
