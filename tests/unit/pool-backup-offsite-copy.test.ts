...
import {
  ...
  planOffsiteCopy,
  verifyOffsiteCopy
} from '../../scripts/pool-backup-offsite-copy.mjs';
...
function encryptedFixture(root: string, name = '20260710T220000Z') {
  ...
  const archivePath = path.join(snapshot, 'pool-backup.tar.gz.age');
...
describe('off-device encrypted backup copies', () => {
  it('plans and verifies only checksum-covered encrypted snapshots', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-offsite-plan-'));
  ...
  it('copies append-only with exact acknowledgement and verifies readback', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-offsite-copy-'));
    const snapshot = encryptedFixture(root);
    const destination = path.join(root, 'destination');
    fs.mkdirSync(destination);
  ...
  it('rejects tampered archives and unsafe receipt paths', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-offsite-tamper-'));
    const snapshot = encryptedFixture(root);
    fs.appendFileSync(path.join(snapshot, 'pool-backup.tar.gz.age'), 'tampered');
