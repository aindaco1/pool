(function() {
  ...
  if (window.PoolPagePrefetch) return;

  var script = document.currentScript || document.querySelector('[data-pool-page-prefetch]');
  ...
  function getLogger() {
    return window.PoolLogger?.createLogger?.('page-prefetch') || {
      debug: function() {},
      info: function() {},
      warn: function() {},
      error: function() {}
  ...
  function isAllowedPublicPath(pathname) {
    ...
    if (path === '/' || path === '/about/' || path === '/terms/' || path === '/creator-campaign-checklist/') {
      return true;
    }
  ...
  function isBlockedPath(pathname) {
    var path = getLocaleStrippedPath(pathname);
    return /^\/(?:admin|manage|community|cart|checkout|checkout-intent|pledge-success|pledge-cancelled|api|worker)(?:\/|$)/.test(path);
  }
  ...
  function prefetchUrl(url, reason) {
    ...
    link.as = 'document';
    link.href = url;
    link.dataset.poolPagePrefetch = 'true';
    if (reason) {
