(function() {
  ...
  if (window.PoolFormControlIdentity?.start) return;
  ...
  function controlIdentityBase(control) {
    ...
    return dataset.contentAction ||
      dataset.action ||
      dataset.itemId ||
      dataset.scrollTarget ||
      dataset.campaignEmbedCopy ||
  ...
  function ensureControlIdentity(control) {
    ...
    control.id = 'pool-form-control-' + slugifyControlPart(controlIdentityBase(control), control.tagName.toLowerCase()) + '-' + String(++controlIdCounter);
  }
  ...
  window.PoolFormControlIdentity = { start: start };
