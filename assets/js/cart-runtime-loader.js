(function() {
  ...
  if (window.PoolCartRuntime) return;

  var loaderScript = document.currentScript || document.querySelector('[data-pool-cart-runtime-loader]');
  var loaderUrl = null;
  try {
  ...
  var runtimeScripts = [
    {
      ...
      key: 'add-on-utils',
      ready: function() {
        return Boolean(window.PoolAddOnUtils);
      }
    },
    {
      ...
      ready: function() {
        return Boolean(window.PoolShippingOptionUtils);
      }
    },
    {
      ...
      ready: function() {
        return Boolean(window.PoolStripeCheckoutSidecar);
      }
    },
    {
      ...
      key: 'cart-provider',
      ready: function() {
        return Boolean(window.PoolCartProvider);
      }
    },
    {
      file: 'cart.js',
      key: 'cart',
      ready: function() {
        return Boolean(window.__PoolCartRuntimeCartUiLoaded);
      }
    },
    {
      file: 'buy-buttons.js',
      key: 'buy-buttons',
      ready: function() {
        return Boolean(window.__PoolBuyButtonsLoaded);
      }
    }
  ];

  function getLogger() {
    return window.PoolLogger?.createLogger?.('cart-runtime') || {
      debug: function() {},
      info: function() {},
      warn: function() {},
      error: function() {}
  ...
  function findExistingRuntimeScript(scriptDef) {
    return document.querySelector('script[data-pool-cart-runtime-script="' + scriptDef.key + '"]');
  }
  ...
  function loadScript(scriptDef) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (!existing) {
        ...
        script.defer = true;
        script.async = false;
        script.dataset.poolCartRuntimeScript = scriptDef.key;
  ...
  async function loadRuntime(reason) {
    if (runtimeLoaded && window.PoolCartProvider) {
      await waitForProvider();
      return window.PoolCartProvider;
    }
    ...
    runtimePromise = runtimeScripts.reduce(function(promise, scriptDef) {
      ...
      .then(function(provider) {
        runtimeLoaded = true;
        dispatchRuntimeEvent('poolcart.runtime.ready', {
          reason: reason || '',
      ...
      .catch(function(error) {
        ...
        dispatchRuntimeEvent('poolcart.runtime.error', {
          reason: reason || '',
  ...
  function hasStoredCartWork() {
    var checks = [
      ...
      ['localStorage', 'pool_first_party_cart_state'],
      ['localStorage', 'pool_first_party_checkout_snapshot'],
      ['localStorage', 'pool_pending_pledge'],
      ['sessionStorage', 'pool_first_party_cart_draft'],
      ['sessionStorage', 'pool_pending_pledge'],
      ['sessionStorage', 'pool_active_custom_checkout_order_id']
    ];
  ...
  function isCartRecoveryPath() {
    ...
    return /^\/(?:[a-z]{2,3}(?:-[a-z0-9]{2,8})?\/)?(?:cart|checkout|pledge-success|pledge-cancelled)\/?$/.test(path) ||
  ...
  async function handleHeaderCartClick(event) {
    ...
    if (runtimeLoaded && window.PoolCartProvider) return;
  ...
  async function handleAddButtonClick(event) {
    var button = event.target?.closest?.('.poolcart-add-item');
    if (!button) return;
  ...
  function getCartIntentTarget(event) {
    var target = event.target?.closest?.('#header-cart-btn, .poolcart-add-item') || null;
    if (target?.matches?.('.poolcart-add-item') && target.disabled) return null;
    return target;
  }
  ...
  window.PoolCartRuntime = {
    load: loadRuntime,
    shouldAutoload: shouldAutoload,
    isLoaded: function() {
      return runtimeLoaded && Boolean(window.PoolCartProvider);
    }
  };
