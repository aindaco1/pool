(function() {
  ...
  function localizeReadOnlySettingsValue(row) {
    ...
    var exact = {
      ...
      'Returns not permitted': 'settings_status_returns_not_permitted'
    };
  ...
  function setAdminLoginStartStatus(data) {
    var loginUrl = String(data?.loginUrl || '').trim();
    if (!loginUrl) {
    ...
    setAuthStatus('');
    authStatus.setAttribute('data-admin-prominent-status', 'true');

    var message = document.createElement('span');
    message.className = 'admin-dashboard__status-message';
    message.textContent = t('dev_link_ready', 'Development login link is ready.');

    var link = document.createElement('a');
    link.className = 'btn btn--secondary btn--small admin-dashboard__status-action';
    link.href = loginUrl;
    link.textContent = t('dev_link_open', 'Open admin');

    authStatus.append(message, link);
  }
  ...
  function createAdminReviewTable(captionText, headers) {
    var wrap = document.createElement('div');
    ...
    wrap.append(table);
    return { wrap: wrap, body: tbody, headers: headers };
  }

  function appendAdminReviewCells(reviewTable, row, values) {
    values.forEach(function(value, index) {
      var td = document.createElement('td');
      td.dataset.label = reviewTable.headers[index] || '';
      td.textContent = value;
      row.append(td);
    });
  }
  ...
  function renderAdminSessions(root, data) {
    ...
    if (!active.length) {
      ...
      var activeTable = createAdminReviewTable(t('admin_sessions_active_caption', 'Active administrator sessions'), [
      ...
      active.forEach(function(session) {
        var row = document.createElement('tr');
        appendAdminReviewCells(activeTable, row, [
          session.email || t('admin_value_unknown', 'Unknown'),
          adminSessionClientLabel(session),
        ...
        if (session.current) {
          ...
          revoke.addEventListener('click', async function() {
            var confirmed = window.confirm(t('admin_sessions_revoke_confirm', 'Revoke this administrator session?'));
            if (!confirmed) return;
            ...
            try {
              ...
              await loadAdminSessionReview(root, { force: true, preserveStatus: true });
            } catch (error) {
    ...
    var recentTable = createAdminReviewTable(t('admin_sessions_recent_caption', 'Recent administrator logins'), [
      t('admin_sessions_admin', 'Admin'),
      t('admin_sessions_client', 'Client'),
    ...
    recent.slice(0, 100).forEach(function(session) {
      var row = document.createElement('tr');
      appendAdminReviewCells(recentTable, row, [
        session.email || t('admin_value_unknown', 'Unknown'),
        adminSessionClientLabel(session),
  ...
  async function loadAdminSessionReview(root, options) {
    if (!(root instanceof HTMLElement)) return;
    ...
    try {
      var data = await requestJson('/admin/sessions', { method: 'GET' });
  ...
  function createAdminSessionReview() {
    ...
    refresh.addEventListener('click', function() {
      loadAdminSessionReview(root, { force: true });
    });
  ...
  function adminAuditIdentifierLabel(value) {
  ...
  var adminAuditActionDefinitions = [
  ...
  function adminAuditCampaignName(campaignSlug) {
    var campaign = currentCampaigns.find(function(item) { return String(item?.slug || '') === campaignSlug; });
    ...
    var syntheticIdentity = [campaignSlug, configuredTitle].find(function(value) {
      ...
      return /(?:^|-)local-no-user(?:-\d{10,})?$/.test(normalized);
    });
    if (syntheticIdentity) {
      return t('admin_audit_target_local_test_campaign_unassigned', 'Local test campaign (unassigned)');
    }
    if (configuredTitle) return configuredTitle;
    var stableSlug = String(campaignSlug || '').replace(/[-_]\d{10,}$/, '');
    return adminAuditIdentifierLabel(stableSlug || campaignSlug);
  }
  ...
  function adminAuditTarget(event) {
    ...
    if (campaignSlug) {
      return {
        label: t('admin_audit_target_campaign', 'Campaign: %{name}', { name: adminAuditCampaignName(campaignSlug) }),
        raw: campaignSlug,
    ...
    if (productId) {
      return {
        label: t('admin_audit_target_product', 'Product: %{name}', { name: adminAuditIdentifierLabel(productId) }),
        raw: productId,
    ...
    if (source) {
      ...
      return {
        label: knownSource
          ? t(knownSource[0], knownSource[1])
          : t('admin_audit_target_source', 'Source: %{name}', { name: adminAuditIdentifierLabel(source) }),
        raw: source,
  ...
  function adminAuditStatus(event) {
    ...
    var knownStatus = knownStatuses[rawStatus.toLowerCase()];
    var parts = [];
    if (rawStatus) parts.push(knownStatus ? t(knownStatus[0], knownStatus[1]) : adminAuditIdentifierLabel(rawStatus));
    var knownFields = {
      content: ['admin_audit_field_content', 'Content'],
      description: ['admin_audit_field_description', 'Description'],
      email: ['admin_audit_field_email', 'Email'],
      goal: ['admin_audit_field_goal', 'Goal'],
      image: ['admin_audit_field_image', 'Image'],
      inventory: ['admin_audit_field_inventory', 'Inventory'],
      permissions: ['admin_audit_field_permissions', 'Permissions'],
      pricing: ['admin_audit_field_pricing', 'Pricing'],
      role: ['admin_audit_field_role', 'Role'],
      shipping: ['admin_audit_field_shipping', 'Shipping'],
      state: ['admin_audit_field_state', 'State'],
      status: ['admin_audit_field_status', 'Status'],
      tiers: ['admin_audit_field_tiers', 'Tiers'],
      title: ['admin_audit_field_title', 'Title']
    };
    var changedFields = (Array.isArray(event.changedFields) ? event.changedFields : [])
      .map(function(field) {
        var rawField = String(field || '').trim();
        var knownField = knownFields[rawField.toLowerCase()];
        return knownField ? t(knownField[0], knownField[1]) : adminAuditIdentifierLabel(rawField);
      })
      .filter(Boolean);
    if (changedFields.length) {
      parts.push(t('admin_audit_status_fields_changed', 'Fields changed: %{fields}', { fields: changedFields.join(', ') }));
    }
  ...
  function adminAuditActionLabel(action) {
    ...
    var known = adminAuditActionDefinitions.find(function(definition) {
      return definition[0] === rawAction.toLowerCase();
    });
    if (known) return t(known[1], known[2]);
    return adminAuditIdentifierLabel(rawAction) || rawAction;
  }
  ...
  function renderAdminAuditRows(root, data) {
    ...
    var summary = document.createElement('p');
    summary.className = 'admin-app__muted';
    summary.textContent = t('admin_audit_summary', '%{count} matching events. Sensitive event payloads are excluded.', {
      count: formatNumber(matched)
    });
    results.append(summary);
    ...
    var auditTable = createAdminReviewTable(t('admin_audit_caption', 'Administrator audit events'), [
      t('admin_audit_time', 'Time'),
      t('admin_audit_action', 'Action'),
      t('admin_audit_admin', 'Admin'),
    ...
    rows.forEach(function(event) {
      ...
      var target = adminAuditTarget(event);
      appendAdminReviewCells(auditTable, row, [
        formatAdminDate(event.createdAt),
        adminAuditActionLabel(rawAction),
        event.adminEmail || t('admin_audit_system', 'System'),
        target.label,
        adminAuditStatus(event)
      ]);
  ...
  async function loadAdminAuditReview(root) {
    if (!(root instanceof HTMLElement)) return;
    ...
    try {
      ...
      var data = await requestJson('/admin/audit' + (query ? '?' + query : ''), { method: 'GET' });
  ...
  async function downloadAdminAuditCsv(root) {
    var status = root.querySelector('[data-admin-audit-status]');
    setText(status, t('admin_audit_exporting', 'Preparing audit CSV...'));
    try {
      var query = adminAuditQuery(root);
      var result = await requestBlob('/admin/audit.csv' + (query ? '?' + query : ''), { method: 'GET' });
      downloadBlobResult(result, 'pool-admin-audit.csv');
  ...
  function createAdminAuditReview() {
    ...
    [
      {
        name: 'date',
        label: t('admin_audit_filter_date', 'Date'),
        type: 'date',
        help: t('admin_audit_filter_date_help', 'Show events recorded on one calendar date.')
      },
      {
        name: 'action',
        label: t('admin_audit_filter_action', 'Action'),
        type: 'action-select',
        help: t('admin_audit_filter_action_help', 'Choose the kind of administrator activity to review.')
      },
      {
        name: 'email',
        label: t('admin_audit_filter_email', 'Admin email'),
        type: 'email',
        placeholder: t('admin_audit_filter_email_placeholder', 'admin@example.com'),
        help: t('admin_audit_filter_email_help', 'Enter the complete email address for one administrator.')
      },
      {
        name: 'campaignSlug',
        label: t('admin_audit_filter_campaign', 'Campaign'),
        type: 'campaign-select',
        help: t('admin_audit_filter_campaign_help', 'Choose one campaign, or leave this set to all campaigns.')
      },
      {
        name: 'q',
        label: t('admin_audit_filter_search', 'Search'),
        type: 'search',
        placeholder: t('admin_audit_filter_search_placeholder', 'Target, status, or changed field'),
        help: t('admin_audit_filter_search_help', 'Search stored targets, outcomes, and changed-field names.')
      }
      ...
      if (field.type === 'action-select' || field.type === 'campaign-select') {
        ...
        if (field.type === 'action-select') {
          adminAuditActionDefinitions.forEach(function(definition) {
            var option = document.createElement('option');
          ...
          currentCampaigns.forEach(function(campaign) {
            ...
            var option = document.createElement('option');
            option.value = slug;
            option.textContent = adminAuditCampaignName(slug);
            control.append(option);
          });
        }
      } else {
  ...
  function appendSettingsControl(parent, row, section, index) {
    ...
    if (row?.input === 'admin-session-review') {
      var sessionReview = createAdminSessionReview();
      describeCompositeControl(sessionReview, describedById, row?.label || '');
      parent.append(sessionReview);
      return;
    }
    if (row?.input === 'admin-audit-review') {
      var auditReview = createAdminAuditReview();
      describeCompositeControl(auditReview, describedById, row?.label || '');
      parent.append(auditReview);
      return;
    }
  ...
  function renderSettingsTable(section, options) {
    ...
    settingsRowsForRender(section?.rows || []).forEach(function(row, index) {
      ...
      var renderFullWidth = options?.fullWidthRows === true || row?.input === 'settings-field-grid' || ['plan-usage', 'performance-observability', 'admin-session-review', 'admin-audit-review'].includes(row?.input);
  ...
  function campaignSettingsSubtabForRow(row) {
    ...
    if (path === 'featured_tier_id' || path === 'shopping.enabled' || path === 'shopping.availability_date' || path === 'tiers') return 'tiers';
  ...
  function selectSettingsSection(sectionId, options) {
    ...
    activePanel?.querySelectorAll?.('[data-admin-session-review]').forEach(loadAdminSessionReview);
    activePanel?.querySelectorAll?.('[data-admin-audit-review]').forEach(function(root) {
      if (root.dataset.adminAuditState !== 'loaded') loadAdminAuditReview(root);
    });
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async function(event) {
      ...
      try {
        ...
        setAdminLoginStartStatus(result);
      } catch (error) {
