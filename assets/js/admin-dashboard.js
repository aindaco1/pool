(function() {
  ...
  function createAdminSettingsReviewApi() {
    ...
    if (!window.PoolAdminSettingsReview?.create) return null;
    adminSettingsReviewApi = window.PoolAdminSettingsReview.create({
      appendDescribedBy: appendDescribedBy,
      appendTableHeader: appendTableHeader,
      createProductLabelRow: createProductLabelRow,
      downloadBlobResult: downloadBlobResult,
      formatNumber: formatNumber,
      getCampaigns: function() { return currentCampaigns; },
      lang: lang,
      logger: logger,
  ...
  function loadAdminSettingsReviewModule() {
    ...
    adminSettingsReviewPromise = new Promise(function(resolve, reject) {
      ...
      var sourceUrl = new URL('admin-settings-review.js', script.src);
      sourceUrl.search = new URL(script.src, document.baseURI).search;
      moduleScript.src = sourceUrl.toString();
  ...
  function createAdminReviewPlaceholder(type) {
    ...
    root.className = sessions ? 'admin-session-review' : 'admin-audit-review';
    ...
    status.textContent = sessions
      ? t('admin_sessions_loading', 'Loading admin sessions...')
      : t('admin_audit_loading', 'Loading audit events...');
    root.append(status);
    return root;
  }

  function activateAdminReview(root, type) {
    if (!(root instanceof HTMLElement)) return;
    if (root.dataset.adminReviewHydrated === 'true') {
      loadAdminSettingsReviewModule().then(function(api) {
        if (type === 'sessions') api.loadSessionReview(root);
        else if (root.dataset.adminAuditState !== 'loaded') api.loadAuditReview(root);
      }).catch(function(error) {
        logger.error('Failed to activate admin settings review', error);
      });
      return;
    }
    if (root.dataset.adminReviewHydration === 'loading') return;
    root.dataset.adminReviewHydration = 'loading';
    loadAdminSettingsReviewModule().then(function(api) {
      if (!root.isConnected) return;
      var review = type === 'sessions' ? api.createSessionReview() : api.createAuditReview();
      ['aria-describedby', 'aria-label'].forEach(function(attribute) {
        var value = root.getAttribute(attribute);
        if (value) review.setAttribute(attribute, value);
      });
      review.dataset.adminReviewHydrated = 'true';
      root.replaceWith(review);
      if (type === 'sessions') api.loadSessionReview(review);
      else api.loadAuditReview(review);
    }).catch(function(error) {
      logger.error('Failed to load admin settings review', error);
      root.dataset.adminReviewHydration = 'failed';
      var status = root.querySelector('[role="status"]');
      setText(status, type === 'sessions'
        ? t('admin_sessions_load_failed', 'Unable to load admin sessions.')
        : t('admin_audit_load_failed', 'Unable to load audit events.'));
    });
  }

  function loadAdminReviews(panel) {
    panel?.querySelectorAll?.('[data-admin-session-review]').forEach(function(root) {
      activateAdminReview(root, 'sessions');
    });
    panel?.querySelectorAll?.('[data-admin-audit-review]').forEach(function(root) {
      activateAdminReview(root, 'audit');
    });
  }

  /* Admin session and audit rendering live in admin-settings-review.js so these
     privileged, infrequently used views do not inflate the initial dashboard bundle. */
