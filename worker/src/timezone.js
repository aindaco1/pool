...
export function campaignStartDate(dateString, envOrTimeZone = {}) {
  return dateAtTimeInTimeZone(dateString, timeZoneFromEnvOrValue(envOrTimeZone), 0, 0, 0);
}

export function campaignDeadlineDate(dateString, envOrTimeZone = {}) {
  return dateAtTimeInTimeZone(dateString, timeZoneFromEnvOrValue(envOrTimeZone), 23, 59, 59);
}

export function isCampaignDeadlinePassed(dateString, envOrTimeZone = {}, now = new Date()) {
  const deadline = campaignDeadlineDate(dateString, envOrTimeZone);
  return deadline instanceof Date && Number.isFinite(deadline.getTime()) && now > deadline;
}
