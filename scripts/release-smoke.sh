...
EVIDENCE_FILE=""
LOG_DIR="${RELEASE_SMOKE_LOG_DIR:-$(mktemp -d /tmp/pool-release-smoke-logs.XXXXXX)}"
...
Options:
  ...
  --skip-readiness       Skip setup/deploy production dry-run readiness.
  ...
  --skip-pledge-evidence
                         Skip Worker-backed pledge/report evidence.
...
Default behavior runs the pre-merge gate, setup/deploy readiness, and Podman E2E
when Podman is available. It also records focused accessibility evidence,
rendered i18n/SEO evidence, Worker-backed pledge/report evidence, read-only
...
write_evidence() {
  ...
  {
    printf '# Pool Release Smoke Evidence\n\n'
    ...
    printf '%s\n' '- [ ] I18N: rendered English and Spanish public/campaign/manage/admin route evidence passed; creator-authored campaign content remains canonical unless explicit localized variants exist.'
    printf '%s\n' '- [ ] Podman: local stack and Podman E2E behavior accepted, or skip reason documented above.'
    printf '%s\n' '- [ ] SEO: rendered canonical, hreflang, social metadata, campaign CreativeWork/Breadcrumb JSON-LD, sitemap, robots, and private-route noindex evidence passed.'
    printf '%s\n' '- [ ] Providers: automated provider probes passed or each skip has owner/date/reason and external evidence.'
    printf '%s\n' '- [ ] Pool checkout, pledge, and reports: automated payment and pledge/report evidence passed or each skip has owner/date/reason; checkout start, Manage Pledge, settlement, pledge/fulfillment CSVs, supporter emails, and failure paths covered.'
    printf '%s\n\n' '- [ ] Admin: settings, add-ons, campaigns, reports, analytics, supporters, marketing, Blast, users, scoped access, Spanish admin, tab persistence, and CSV exports reviewed.'
    ...
    printf '%s\n' '- External provider evidence is required for any Cloudflare, Stripe, Resend, USPS, ZIP.TAX, Turnstile, or DNS surface skipped by automated provider probes.'
    printf '%s\n\n' '- Any skipped Podman, payment, accessibility, provider, i18n/SEO, or pledge/report check must include owner, date, reason, and follow-up.'
    printf '## Sign-Off\n\n'
    ...
    printf '%s\n' '- Podman owner/date:'
    printf '%s\n' '- SEO owner/date:'
    printf '%s\n' '- Checkout/pledge/report owner/date:'
    printf '%s\n' '- Admin owner/date:'
    printf '%s\n' '- Notes/blockers:'
  } > "$EVIDENCE_FILE"
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --skip-i18n-seo-evidence)
      ...
      shift
      ;;
    --skip-pledge-evidence|--skip-fulfillment-evidence)
      RUN_PLEDGE_EVIDENCE=false
      shift
      ;;
    --screen-reader-evidence)
...
if [ "$RUN_READINESS" = "true" ]; then
  run_required_phase "2. Setup/deploy readiness dry run" npm run setup:deploy -- --mode=production --dry-run --non-interactive --skip-auth --skip-secrets
else
  record_skip "2. Setup/deploy readiness dry run" "Skipped by --skip-readiness"
fi
...
if [ "$RUN_PLEDGE_EVIDENCE" = "true" ]; then
  run_required_phase "7. Worker pledge/report evidence" npm run release:pledge-evidence
else
  record_skip "7. Worker pledge/report evidence" "Skipped by --skip-pledge-evidence"
fi
