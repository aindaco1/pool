...
const ACKNOWLEDGEMENT = 'POOL_BACKUP_OFF_DEVICE_COPY';
...
export function verifyOffsiteCopy(options = {}) {
  ...
  if (options.verifyDecrypt === true) {
    ...
    const identity = path.resolve(String(options.ageIdentity || process.env.POOL_BACKUP_AGE_IDENTITY || ''));
    if (!fs.existsSync(identity) || !fs.lstatSync(identity).isFile()) {
      throw new Error('Set POOL_BACKUP_AGE_IDENTITY to a real identity file for decryption verification.');
    }
...
function printHelp() {
  console.log(`Usage: node scripts/pool-backup-offsite-copy.mjs --snapshot=DIR [options]
...
Options:
  ...
  --verify-decrypt      Also decrypt to /dev/null using POOL_BACKUP_AGE_IDENTITY.
...
async function main() {
  ...
  if (args.includes('--verify-only')) {
    result = verifyOffsiteCopy({
      ...
      ageIdentity: process.env.POOL_BACKUP_AGE_IDENTITY || ''
    });
