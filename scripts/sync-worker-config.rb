...
def build_mirror_values(config, existing)
  ...
  launch_reminders = config['launch_reminders'] || {}
  debug = config['debug'] || {}
...

  {
    ...
    'CAMPAIGN_RUNNER_REPORT_HOUR' => campaign_runner_reports.key?('send_hour') ? format_int(campaign_runner_reports['send_hour']) : campaign_runner_reports.key?('send_hour_mt') ? format_int(campaign_runner_reports['send_hour_mt']) : existing['CAMPAIGN_RUNNER_REPORT_HOUR'] || existing['CAMPAIGN_RUNNER_REPORT_HOUR_MT'],
    'CAMPAIGN_RUNNER_REPORT_MINUTE' => campaign_runner_reports.key?('send_minute') ? format_int(campaign_runner_reports['send_minute']) : campaign_runner_reports.key?('send_minute_mt') ? format_int(campaign_runner_reports['send_minute_mt']) : existing['CAMPAIGN_RUNNER_REPORT_MINUTE'] || existing['CAMPAIGN_RUNNER_REPORT_MINUTE_MT'],
    ...
    'LAUNCH_REMINDERS_ENABLED' => launch_reminders.key?('enabled') ? (launch_reminders['enabled'] ? 'true' : 'false') : existing['LAUNCH_REMINDERS_ENABLED'],
