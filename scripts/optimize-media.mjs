...
const REFERENCE_ROOTS = ['_campaigns', '_data', '_includes', '_layouts'];
...
async function buildMediaReferenceIndex(paths = []) {
  ...
  for (const filePath of await referenceFiles()) {
    ...
    for (const repoPath of paths) {
      ...
      if (count > 0) {
        index.get(repoPath).push({ path: normalizeRepoPath(filePath), count });
      }
    }
  }
  return index;
...
export async function buildMediaOptimizationManifest(tools = {}, { previousManifest = {}, optimizerResults = [] } = {}) {
  ...
  const classified = repoPaths
    .map((repoPath) => classifyMediaPath(repoPath, knownPaths))
    .filter(Boolean);
  const sourceRows = classified.filter((asset) => asset.role === 'source');
  const referenceIndex = await buildMediaReferenceIndex(repoPaths);
  ...
  for (const source of sourceRows) {
    ...
    const metadata = await probeMediaMetadata(source.path, tools);
