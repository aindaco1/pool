...
import { auditPoolDataInventory } from './audit-pool-data-inventory.mjs';
import { commandAvailable } from './lib/command-runner.mjs';
import { buildPoolSecretInventory, createPoolBackup } from './pool-backup.mjs';
...
export async function collectBackupReadiness(options = {}) {
  const checks = [];
  const inventory = auditPoolDataInventory();
  checks.push({
    id: 'data-inventory',
    status: inventory.ok ? 'PASS' : 'FAIL',
    detail: inventory.ok
      ? 'required Pool Worker storage families are classified'
  ...
  let backupPlan = null;
  try {
    backupPlan = await createPoolBackup({
      dryRun: true,
      remote: false,
      skipGitBundle: true,
      skipBuild: true
    ...
    checks.push({
      ...
      detail: `snapshot v${backupPlan.version || 0} planned with ${backupPlan.kvPlan?.length || 0} classified KV families`
    });
  } catch (error) {
    checks.push({
      id: 'metadata-backup-plan',
  ...
  const secrets = buildPoolSecretInventory({ env: options.env || process.env, devVarsPath: options.devVarsPath });
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log('Usage: node scripts/pool-backup-readiness.mjs [--provider-evidence=FILE] [--snapshot-receipt=FILE] [--rehearsal-evidence=FILE] [--max-snapshot-age-hours=24] [--max-rehearsal-age-hours=2160] [--require-current-evidence] [--output=FILE] [--strict]');
