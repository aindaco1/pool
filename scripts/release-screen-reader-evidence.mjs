...
console.log('Pool release screen-reader evidence');
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pool-screen-reader-evidence-'));
...
if (audioFile) {
  if (!fs.existsSync(audioFile)) {
    ...
    assertTranscript(transcript, expected.length ? expected : ['The Pool']);
  }
