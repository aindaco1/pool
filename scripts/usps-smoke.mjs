...
import { quoteCampaignShipment, __resetShippingRuntimeStateForTests } from '../worker/src/shipping.js';
...
const scenarios = [
  {
    ...
    args: (env) => [env, smokeCampaign, posterShipment, { country: 'US', postalCode: '80205' }]
  },
  {
    name: 'domestic_physical_tier_signature_required',
    args: (env) => [env, smokeCampaign, posterShipment, { country: 'US', postalCode: '80205' }, [], 'signature_required']
  },
  {
    name: 'international_physical_tier_standard',
    args: (env) => [env, smokeCampaign, posterShipment, { country: 'CA', postalCode: 'M5V 2T6' }]
  },
  {
    name: 'campaign_add_on_only_with_digital_tier',
    args: (env) => [env, smokeCampaign, digitalShipment, { country: 'US', postalCode: '80205' }, [], 'standard', [campaignPosterAddOn]]
  },
  {
    name: 'platform_global_add_ons_only',
    args: (env) => [env, null, { selectedTiers: [] }, { country: 'US', postalCode: '80205' }, [], 'standard', [platformStickerAddOn]]
  }
];
...
for (const scenario of scenarios) {
  ...
  const result = await quoteCampaignShipment(...scenario.args(env));
