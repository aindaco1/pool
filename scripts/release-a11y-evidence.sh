...
GREP_PATTERN="${RELEASE_A11Y_GREP:-has no obvious axe violations|200% text scaling|supports keyboard-only|release focus order|release live status|release reduced motion}"
HOST_SERVER_PID=""
...
The focused evidence path covers automated axe checks, public keyboard-only
campaign/cart flows, release-critical 200% text-scaling checks, focus-order
reachability, live status regions, and reduced-motion checks. VoiceOver/Whisper
transcript evidence is optional unless the release scope explicitly requires it.
EOF
}
...
ensure_host_server() {
  ...
  python3 -m http.server 4002 --bind 127.0.0.1 --directory _site >/tmp/pool-release-a11y-server.log 2>&1 &
  HOST_SERVER_PID="$!"
  if ! wait_for_host_server; then
    echo "Host accessibility server failed to start; log follows:" >&2
    cat /tmp/pool-release-a11y-server.log >&2 || true
    return 1
  fi
}
...
echo "Pool release accessibility evidence"
echo "Mode: $MODE"
...
if [ "$MODE" = "podman" ]; then
  ...
  ensure_host_server
  CI=1 PLAYWRIGHT_WORKERS=1 PLAYWRIGHT_EXTERNAL_SERVER=1 PLAYWRIGHT_BASE_URL=http://127.0.0.1:4002 npx playwright test \
    tests/e2e/accessibility-public-pages.spec.ts \
    tests/e2e/release-a11y-evidence.spec.ts \
    tests/e2e/admin-dashboard.spec.ts \
    tests/e2e/public-page-controls.spec.ts \
