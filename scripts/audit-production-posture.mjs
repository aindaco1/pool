...
export function auditProductionPosture({ config, inventory, secrets = [], providerEvidence = null }) {
  ...
  for (const binding of config.requiredKvBindings || []) {
    add(`binding:${binding}`, kv.has(binding) ? 'ok' : 'action', kv.has(binding) ? 'Configured' : 'Missing KV binding');
  }
  for (const binding of config.requiredDurableObjectBindings || []) {
    add(`binding:${binding}`, durable.has(binding) ? 'ok' : 'action', durable.has(binding) ? 'Configured' : 'Missing Durable Object binding');
  }
