...
export function buildRecoveryTrafficQuery() {
  return `query PoolRecoveryTraffic($accountTag: string!, $scriptName: string!, $start: string!, $end: string!) {
  viewer {
...
export async function collectRecoveryTrafficPreflight(options = {}) {
  ...
  const scriptName = normalizedScriptName(options.scriptName || 'pledge-worker');
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log('Usage: node scripts/pool-recovery-traffic-preflight.mjs [--recent-minutes=15] [--maximum-requests=100] [--maximum-errors=0] [--output=FILE] [--strict]');
  ...
  const result = await collectRecoveryTrafficPreflight({
    ...
    scriptName: process.env.CLOUDFLARE_WORKER_SCRIPT_NAME || 'pledge-worker',
