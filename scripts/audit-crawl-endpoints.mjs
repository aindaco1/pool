...
const DEFAULT_BASE_URL = 'https://pool.dustwave.xyz';
...
const PRIVATE_PATH_PATTERNS = [
  ...
  /^\/(?:es\/)?api(?:\/|$)/,
  /^\/(?:es\/)?embed(?:\/|$)/,
  /^\/(?:es\/)?pledge-(?:success|cancelled)(?:\/|$)/,
  /^\/(?:es\/)?campaigns\/[^/]+\/preview(?:\/|$)/,
  /^\/(?:es\/)?community\/[^/]+(?:\/|$)/
];
...
async function auditOnce({ baseUrl, fetchFn }) {
  const errors = [];
  const ordinary = await fetchText(fetchFn, `${baseUrl}/sitemap.xml`, 'Pool crawl endpoint audit/1.1');
