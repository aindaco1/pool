...
endpoint, Resend sender domain, KV, and USPS readiness. DNS evidence can use a
...
function expectedEmailDomains(vars) {
  ...
  for (const key of ['PLEDGES_EMAIL_FROM', 'UPDATES_EMAIL_FROM']) {
...
function hasRequiredStripeWebhook(endpoint, workerBase) {
  const requiredEvents = new Set(['checkout.session.completed', 'payment_intent.payment_failed']);
...
async function main() {
  ...
  const siteBase = vars.SITE_BASE || 'https://pool.dustwave.xyz';
  const workerBase = vars.WORKER_BASE || 'https://pledge.dustwave.xyz';
  ...
  await checkDns(siteHost, 'Public site DNS');
  ...
  if (stripeLiveCli.ok) {
    ...
    else add('FAIL', 'Stripe production webhook endpoint', 'live endpoint not found with required checkout/session and payment failure events');
  } else {
