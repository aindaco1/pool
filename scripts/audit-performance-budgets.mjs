...
export function collectAssetBudgetEvidence(options = {}) {
  ...
  const checks = [
    { id: 'javascript-total', actual: javascriptTotalBytes, maximum: Number(config.assets.javascriptTotalBytes), ok: javascriptTotalBytes <= Number(config.assets.javascriptTotalBytes) },
    { id: 'css-total', actual: cssTotalBytes, maximum: Number(config.assets.cssTotalBytes), ok: cssTotalBytes <= Number(config.assets.cssTotalBytes) }
  ];
  for (const [relativePath, maximum] of Object.entries(config.assets.files || {})) {
    ...
    checks.push({ id: `file:${relativePath}`, actual, maximum: Number(maximum), ok: actual !== null && actual <= Number(maximum) });
  }
  return {
    schemaVersion: 1,
    ...
    workersCache: config.workersCache,
    checks,
