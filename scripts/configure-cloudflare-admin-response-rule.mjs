...
export const ADMIN_RESPONSE_RULE_REF = 'pool_admin_no_transform_v1';
export const ADMIN_RESPONSE_RULE_DESCRIPTION = 'Pool admin no-transform and no-store';
  ...
  if (options.apply === true && state !== 'current') {
    if (!ruleset) {
      ...
      ruleset = await cloudflareRequest({
        ...
        method: 'POST',
        body: {
          name: 'Pool cache response rules',
          description: 'Pool-managed response cache controls',
          kind: 'zone',
...
async function main() {
  ...
  const siteBase = valueArg(args, '--site-base', process.env.SITE_BASE || 'https://pool.dustwave.xyz');
