...
export STAGING_WORKER_URL="https://pledge-staging.dustwave.xyz"
export ADMIN_SECRET="..."
```
...
12. In **Marketing**, save/edit/delete a referral code, verify the URL builder clears after save/refresh, confirm the QR preview updates from the current campaign URL, download PNG/SVG QR files, use **Save shared draft** / **Load shared draft** / **Clear shared draft**, confirm referral/UTM reporting loads from indexed pledges, confirm abandoned-checkout health loads without supporter PII, and confirm the embedded campaign builder still works.
13. In **Campaigns -> Blast**, draft a supporter email blast with text plus a hosted image, selected existing image, or YouTube/Vimeo block; use **Save shared draft** / **Load shared draft** / **Clear shared draft**; click **Send test** and verify the automatic dry-run returns an audience count/hash before the test email goes to the signed-in admin. Then click **Send blast**, confirm the live send, and verify sent history records subject, content, CTA Button Label, and CTA Button URL below the editor. Use a campaign with a rebuilt `campaign-pledges:<slug>` index; missing indexes should fail closed with `campaign_index_required` before any email send.
14. For `/es/admin/`, verify translated tab labels, Plan usage labels/links, Create new campaign / Preview copy, and tablet/mobile navigation do not overflow.
