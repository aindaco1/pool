...
- Focused Worker operations, outbox, and dashboard suites: 155 passed before
  the additional manual-send compatibility test; the final Worker/outbox run
  passed 47 tests, including that compatibility check.
- Final full local unit suite: 1,036 passed, one existing skip. Security: 127 passed.
...
- The first full pre-merge attempt passed through the host Worker smoke, then
  ...
  the gate. The rerun passed every phase, including the Podman mutable-pledge
  smoke. Browser suite: 132 passed, two existing checkout cases passed on retry,
  and three skipped. The new upload case passed on its first attempt.

## Merge and production acceptance

- [Hosted Merge Smoke](https://github.com/aindaco1/pool/actions/runs/36050122473)
  passed the complete gate and all four dependency audits on `d974dda`.
  [PR #62](https://github.com/aindaco1/pool/pull/62) merged as
  `127e3c2975519282693f5b424fa7e0a995c2e2d2`.
- [Deploy Production](https://github.com/aindaco1/pool/actions/runs/36051160901)
  successfully deployed that exact merged revision to the Worker and Pages.
  Worker version: `e42deedc-17cd-421a-9904-5f4c2240c97a`.
  The workflow's admin response security check and all 18 sitemap crawl URLs
  passed. The separate production cache-policy audit passed all 11 targets.
- The served, minified dashboard script contains the campaign-scoped add-on
  upload kind and editor campaign slug. The user should save their draft and
  refresh the dashboard before retrying an upload in an already-open tab.
  An upload from the recipient's physical phone has not been observed.
- At 19:57:02 UTC the new Worker ran the daily report pass outside the former
  exact-minute window. `cron:campaign-runner-reports:2026-09-24` records success
  at 19:57:03. The earlier manual September 24 sent marker remains unchanged.
  A subsequent provider read shows only the two recovered reports, both
  delivered, with no extra report generated by deployment.
- The next morning's scheduled send has not occurred at sign-off. Live evidence
  establishes the repaired same-day eligibility/completion path; regression
  tests cover delayed/missed events, local date changes, daylight saving time,
  failures, deduplication, and explicit manual-send compatibility.
- Rollback is the prior reviewed revision `166f583`. Preserve sent markers and
  delivery evidence. Recovery changed no pledge or payment records.
